...
if __name__ == "__main__":
    ...
    config.add(
        "--upstream-org",
        env_var="UPSTREAM_ORG",
        help="The upstream github organization",
        required=True,
    )
    config.add(
        "--upstream-repo",
        env_var="UPSTREAM_REPO",
        help="The upstream repository",
        required=True,
    )
    config.add(
        "--upstream-branch",
        env_var="UPSTREAM_BRANCH",
        help="The upstream branch",
        required=True,
    )
    config.add(
        "--origin-branch",
        env_var="ORIGIN_BRANCH",
        help="The downstream branch that will be pushed to the " "origin remote",
        required=True,
    )
    ...
    try:
        ...
        if options.upstream_branch not in upstream.refs:
            raise SyncException(
                "Upstream branch ({}) does not exist.".format(options.upstream_branch)
            )

        # Origin branch does not exist in local repo
        if options.origin_branch not in repo.heads:
            repo.create_head(
                options.origin_branch, upstream.refs[options.upstream_branch]
            )
        ...
        if options.origin_branch in origin.refs:
            ...
            origin_commit = None
            print(
                "Origin branch ({}) does not exist on remote, creating it.".format(
                    options.origin_branch
                )
            )
        ...
        if upstream_commit == origin_commit:
            ...
            repo.heads[options.origin_branch].reference = upstream.refs[
